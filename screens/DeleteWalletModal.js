...
export default class DeleteWalletModal extends React.Component {
    ...
    componentWillMount(){
        if (Platform.OS === 'ios'){
            IosWallet.getWalletPin(this.props.navigation.state.params.walletIndexToDelete, (err, walletpin)=> {
                this.setState({isLoading:false, walletpin: walletpin })
            });
        }
        // android
        else {
            AndroidWallet.getWalletPin(this.props.navigation.state.params.walletIndexToDelete, (err) => {console.log(err); }, (walletpin) => {
                this.setState({isLoading:false, walletpin: walletpin })
            })
        }
    }
        ...
        if (this.state.isLoading){
            ...
            return(
                <ImageBackground source={require('../resources/images/complete_setup_bg.png')} style={styles.backgroundImage}>
                    ...
                    subtitleEnter = "to remove your wallet"
                    stylePinCodeColorTitle="white"
                    colorPassword="white"
                    numbersButtonOverlayColor="white"
                    finishProcess = { this.deleteWallet }
                    />
                </ImageBackground>
            );
        }
